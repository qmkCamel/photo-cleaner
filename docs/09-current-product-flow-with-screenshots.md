...
- 点缩略图切换是否加入删除选择。
- 点缩略图左下角更多操作可选择 `确认保留，不再提醒`。确认后该照片立即退出删除选择并显示可撤销反馈；后续扫描不再把它作为普通候选。
- 点 `复核本组全部` 可快速选择本组候选。
- 点 `加入复核箱` 后进入复核箱。
...
入口来自首屏 `了解更多` 或扫描结果后的 `设置` Tab。这里解释本地处理、无上传、无跟踪、用户控制，以及 iCloud Photos 风险。`已确认保留` 管理项展示本机保护数量，可查看缩略图、逐项取消或全部重置；取消后照片可再次进入后续扫描候选。
